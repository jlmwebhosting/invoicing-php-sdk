...
<xsl:template match="*[local-name()='complexType']">
    <class name="{@name | ../@name}">
    	<xsl:if test="./*[local-name()='annotation']/*[local-name()='documentation']">    	
	    	<documentation>
                ...
                <xsl:for-each select=".//*[local-name()='element']">      
                    <xsl:choose>
                        <xsl:when test="substring-before(@type,':')='xsd'">
                            <property name="{@name}" type="{substring-after(@type,':')}" min="{@minOccurs}" max="{@maxOccurs}" />
                        </xsl:when>
                        ...
                        <xsl:when test="not(@type) and @ref">
                            <xsl:choose>
                                <xsl:when test="@name">
                                    <property name="{@name}" type="{substring-after(@ref,':')}"  min="{@minOccurs}" max="{@maxOccurs}"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <property name="{substring-after(@ref,':')}" type="{substring-after(@ref,':')}"  min="{@minOccurs}" max="{@maxOccurs}"/>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:when>
                        <xsl:otherwise>
...
<xsl:template match="*[local-name()='operation' and namespace-uri()='http://schemas.xmlsoap.org/wsdl/']">
    <function name="{@name}">
        ...
        <parameters>
            <xsl:apply-templates select="//*[local-name()='message' and @name=substring-after(current()/*[local-name()='input']/@message, ':')]" />
        </parameters>
        <returns>
            <xsl:apply-templates select="//*[local-name()='message' and @name=substring-after(current()/*[local-name()='output']/@message, ':')]" />
        </returns>        
    </function>
</xsl:template>
